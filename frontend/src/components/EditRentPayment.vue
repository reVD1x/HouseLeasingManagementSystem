...
const form = ref({ id: null, contractId: null, dueDate: '', amount: null, status: 'PENDING', periodStart: '', periodEnd: '', penalty: null })
const loaded = ref(false)

async function load() {
  try {
    const res = await get(`/rent-payments/${id}`)
  form.value = { id: res.id, contractId: res.contractId, dueDate: res.dueDate, amount: res.amount, status: res.status || 'PENDING', periodStart: res.periodStart || '', periodEnd: res.periodEnd || '', penalty: res.penalty || null }
    loaded.value = true
  } catch (e) {
...
async function submit() {
  try {
    ...
    await put(`/rent-payments/${id}`, payload)
